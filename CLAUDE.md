...
## ループ: Design → Discussion → Build → Play（お父さんはメールで見るだけ）

4 フェーズ + Reflect で回る。Play の後に「次はこうしたい」を Issue に書く行為が
Design に戻る接点（= Reflect）。
...
└────────────────────────┬───────────────────────────────┘
                         ↓ 自動: implement-idea.yml（"Go" で発火）
┌─ 🔨 BUILD ─────────────────────────────────────────────┐
│ 🔨 Claude が実装 → master に push → 📧 お父さんに通知    │
│         ↓ nightly-build.yml（push 時 / 手動 / 週1）      │
│ ⚙️ .mcworld + .mcaddon を自動ビルドして Release に公開   │
└────────────────────────┬───────────────────────────────┘
                         ↓
┌─ 🎮 PLAY ──────────────────────────────────────────────┐
│ 📱 iDoraPad で DL → Realm → 🎮 Switch でプレイ           │
└────────────────────────┬───────────────────────────────┘
                         ↓ Reflect
                  （次のアイデアへ → DESIGN に戻る）
```

### 言語化トレーニングとしての Discussion
4 フェーズの核心は **Discussion**（interpret → Go）。
晄希が書いた文章を Claude が構造化し直すことで、
「自分の言語化がどう伝わったか」の差分が可視化される。
その差分を修正指示として書き直す行為が、言語化能力を鍛える。

Play の後の Reflect（「次はこう変えたい」）も、
実装結果と自分のビジョンの差分を言葉にするトレーニング。
